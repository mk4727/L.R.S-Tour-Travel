...
const STORAGE_KEY_AUTH = "rideswift_admin_auth";
...
const AdminDashboard = ({ onLogout }: { onLogout: () => void }) => {
  ...
  const saveCars = async () => {
    for (const car of cars) {
      await supabase.from("cars").upsert({
        ...
        price_per_km: car.price_per_km, fuel_type: car.fuel_type, ac: car.ac,
  ...
  const addCar = async () => {
    const { data } = await supabase.from("cars").insert({
      name: "New Car", type: "Sedan", seats: 4, price_per_km: 10,
            ...
            <div className="p-6 rounded-xl bg-card border border-border space-y-6">
              ...
              <div className="border-t border-border pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div><label className="text-xs text-muted-foreground">Instagram URL</label><Input value={siteSettings.instagram} onChange={(e) => updateSetting("instagram", e.target.value)} placeholder="https://instagram.com/..." /></div>
