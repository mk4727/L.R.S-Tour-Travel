...
const YouTubeIcon = () => (
  <svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
);
...
const Footer = () => {
  ...
  const socialLinks = [
    { icon: WhatsAppIcon, url: settings.whatsapp, label: "WhatsApp" },
    { icon: InstagramIcon, url: settings.instagram, label: "Instagram" },
    { icon: TwitterIcon, url: settings.twitter, label: "Twitter" },
    { icon: FacebookIcon, url: settings.facebook, label: "Facebook" },
    { icon: YouTubeIcon, url: settings.youtube, label: "YouTube" },
  ].filter((s) => s.url);

  return (
    <footer className="py-16 bg-[hsl(220,20%,12%)] text-white">
...
/>
            <span className="text-lg font-display font-bold text-white">
              {settings.companyName.includes("Swift") ? (
                <>{settings.companyName.replace("Swift", "")}<span className="text-primary">Swift</span></>
              ) : (
        ...
        <div className="space-y-4">
          ...
          <ul className="space-y-2 text-sm text-white/70">
            ...
            <li><Link to="/tours" className="hover:text-primary transition-colors">Tour Packages</Link></li>
            <li><Link to="/book" className="hover:text-primary transition-colors">Book a Ride</Link></li>
            <li><Link to="/category/rental" className="hover:text-primary transition-colors">Rental Cars</Link></li>
            <li><Link to="/category/luxury" className="hover:text-primary transition-colors">Luxury Cars</Link></li>
          </ul>
        </div>
        ...
        <div className="space-y-4">
          ...
          <ul className="space-y-2 text-sm text-white/70">
            <li className="flex items-center gap-2"><Phone className="h-4 w-4 text-primary" /> {settings.phone}</li>
            <li className="flex items-center gap-2"><Mail className="h-4 w-4 text-primary" /> {settings.email}</li>
          ...
          <div className="flex flex-wrap gap-3">
            {socialLinks.map((s) => (
              <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer" aria-label={s.label}
                className="h-10 w-10 rounded-full bg-white/10 flex items-center justify-center text-white/70 hover:bg-primary hover:text-primary-foreground transition-all">
                <s.icon />
              </a>
            ))}
          </div>
          <p className="text-xs text-white/50">Connect with us on social media</p>
        </div>
      </div>

      <div className="container mx-auto mt-10 pt-6 border-t border-white/10 text-center text-xs text-white/40">
